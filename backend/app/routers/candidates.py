...
async def list_candidates(constituency_id: UUID):
     # First, retrieve the constituency parties associated with the provided constituency_id
    query_parties = "SELECT constituency_party_id FROM constituency_parties WHERE constituency_id=%s"
    ...
    query_candidates = "SELECT * FROM candidates WHERE constituency_party_id IN %s ALLOW FILTERING"
